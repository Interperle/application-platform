...
export async function saveImageUploadAnswer(
  ...
  if (file) {
    ...
    if (reqtype == "created") {
      ...
      const createBucketEntry = await supabase.storage.from(bucket_name).update(`${(await supabase.auth.getUser()).data.user!.id}_${getOldImageUploadAnswerResponse.data?.imagename}`, file)
      console.log(createBucketEntry)
    }
  }
}
