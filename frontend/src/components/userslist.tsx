...
import { changeRoleOfUser, toggleStatusOfUser, userData } from "@/actions/admin";
...
interface UserListProps {
  ...
  const UserList: React.FC<UserListProps> = ({ users }) => {
    ...
    const handleRoleChange = async (user: userData, newRoleId: UserRole) => {
      ...
      if (updatedUser) {
          dispatch(changeUserRole({ userId: updatedUser.id, newRole: updatedUser.userrole }));
      } else {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {users.map(user => (
              <tr key={user.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{user.last_sign_in_at || 'N/A'}</td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{user.updated_at}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                <UserSpecificRoleDropdown
                  user={user}
                  onRoleChange={(newRole: UserRole) => handleRoleChange(user, newRole).catch(console.error)}
                  />
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><ToggleSwitch isActive={user.isactive} onClick={() => handleToggle(user)}/></td>
              </tr>
            ))}
          </tbody>
        </table>
